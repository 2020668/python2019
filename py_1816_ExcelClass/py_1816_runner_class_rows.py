...
class ReadExcel(object):
    ...
    def read_data(self):
        ...
        for case in list(self.sheet.rows)[1:]:   # 按行获取数据，第2行开始
            if list(self.sheet.rows)[1:].index(case)+1 in self.list:    # 第1行的下标是0，+1与行对齐，来读取指定的行
                ...
                case_data = list(zip(titles, data))     # 打包，标题与数据一一对应。标题只有3个，遍历每行数据也是3个
                for item in case_data:
                    setattr(case_obj, item[0], item[1])     # 对象，属性名，属性值。每行生成一个用例对象
                cases.append(case_obj)
        return cases


if __name__ == '__main__':
    list1 = [3, 5, 8]   # 测试指定行的测试用例,标题不计入行数
